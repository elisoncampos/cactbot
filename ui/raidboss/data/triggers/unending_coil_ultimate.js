    ...
    triggers: [
      ...
      },
      {
        ...
        infoText: function(data, matches) {
          if (!data.hatch) return;
          let hatches = data.hatch
            .map(function(n) {
...

      {
        ...
        regex: /:Thunderwing:26C7:.*?:........:(\y{Name}):/,
        run: function(data, matches) {
          data.thunder = data.thunder || [];
          data.thunder.push(matches[1]);
        }
      },
      {
        id: "UCU Thunder Callouts",
        regex: /:Thunderwing:26C7:.*?:........:(\y{Name}):/,
        delaySeconds: 0.25,
        infoText: function(data, matches) {
          if (!data.thunder) return;
          let thunders = data.thunder
            .map(function(n) {
              return data.ShortName(n);
            })
            .join(", ");
...

      {
        ...
        durationSeconds: function(data, matches) {
          if (parseFloat(matches[2]) <= 6) return 3;

          if (parseFloat(matches[2]) <= 10) return 6;
        ...
        tts: function(data, matches) {
          if (parseFloat(matches[2]) <= 6) return "1";

          if (parseFloat(matches[2]) <= 10) return "2";
      ...
      },
      {
        ...
        run: function(data, matches) {
          ...
          let order = null;
          if (parseFloat(matches[2]) < 9) order = 0;
          else if (parseFloat(matches[2]) < 14) order = 1;
          else order = 2;
      ...
      },
      {
        ...
        infoText: function(data) {
          ...
          if (tookTwo.indexOf(data.me) >= 0) return;
          ...
          if (tookTwo.length > 0) {
            str +=
              ...
              tookTwo
                ...
                .join(", ");
            if (data.lang == "fr") str += " éviter)";
            else if (data.lang == "de") str += " raus)";
            else str += " out)";
          }
          return {
            en: "Fire IN" + str,
        ...
        alertText: function(data) {
          ...
          if (
            data.fireballs[1].indexOf(data.me) >= 0 &&
            data.fireballs[2].indexOf(data.me) >= 0
          ) {
            return {
              en: "Fire IN: AVOID!",
      ...
      },
      {
        ...
        run: function(data, matches) {
          ...
          else dir_names = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"];
          ...
          console.log(
            data.naelMarks.join(", ") + (data.wideThirdDive ? " (WIDE)" : "")
          );
        }
      },
      {
        ...
        infoText: function(data) {
          ...
          return {
            en:
              "Marks: " +
              data.naelMarks.join(", ") +
              (data.wideThirdDive ? " (WIDE)" : ""),
            fr:
              "Marque : " +
              data.naelMarks.join(", ") +
              (data.wideThirdDive ? " (LARGE)" : ""),
            de:
              "Markierungen : " +
              data.naelMarks.join(", ") +
              (data.wideThirdDive ? " (GROß)" : ""),
            ja:
              "マーカー: " +
              data.naelMarks.join(", ") +
              (data.wideThirdDive ? " (広)" : "")
          };
        }
      },
      {
      ...
      },
      {
        ...
        run: function(data, matches) {
          ...
          if (data.octetMarker.length != 7) return;

          let partyList = Object.keys(data.partyList);

          if (partyList.length != 8) {
            console.error(
              "Octet error: bad party list size: " + JSON.stringify(partyList)
            );
            return;
          }
          let uniq_dict = {};
          ...
          if (remainingPlayers.length != 1) {
            ...
            console.error(
              "Octet error: failed to find player, " +
                JSON.stringify(partyList) +
                " " +
      ...
      },
      {
        ...
        infoText: function(data, matches) {
          return {
            en:
              data.octetMarker.length +
              ": " +
              data.ShortName(matches[1]) +
              " (nael)",
            fr:
              data.octetMarker.length +
              " : " +
        ...
        infoText: function(data) {
          ...
          if (data.lastOctetMarker != data.me) {
            return {
              ...
              ja:
                "8: " + data.ShortName(data.lastOctetMarker) + " (ツインタニア)"
            };
          }
        },
        tts: function(data) {
      ...
      },
      {
        ...
        infoText: function(data) {
          if (
            (data.trio != "blackfire" && data.trio != "octet") ||
            data.megaStack.length != 4
          )
            return;

          if (data.megaStack.indexOf(data.me) >= 0) return;
      ...
      },
      {
        ...
        alertText: function(data) {
          if (data.trio == "quickmarch") {
            if (data.shakers.length != 3) return;
            if (data.role == "tank") {
              return {
                en: "Pick up tether",
                fr: "Prendre un lien",
        ...
        run: function(data) {
          ...
          let badSpots = function(mark, dragon) {
            ...
            if ((mark + distance + 8) % 8 == dragon) {
              // Clockwise.
              for (let i = 0; i <= distance; ++i) bad.push((mark + i) % 8);
              if (distance == 1) bad.push((mark - 1 + 8) % 8);
            } else {
              // Widdershins.
              for (let i = 0; i <= distance; ++i) bad.push((mark - i + 8) % 8);
              if (distance == 1) bad.push((mark + 1) % 8);
            }
            return bad;
          };
          ...
          let findDragonMarks = function(array) {
            ...
            let dragons = [];
            for (let i = 0; i < 8; ++i) {
              if (array[i]) dragons.push(i);
            }

            if (dragons.length != 5) return ret;
